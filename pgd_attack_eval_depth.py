...
class PGDAttack:
    ...
    def process(self, epsilon, num_workers=12):
        ...
        self.save_dir = os.path.join(self.eval_out_dir, "pgd_eval_depth", "adv_" + str(epsilon))
    ...
    def evaluate(self, dataloader, pose_dir, epsilon):
        ...
        for i, data in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            with torch.no_grad():
                if i == 0:
                    ...
                    pred_disp = self.disp_net(ref1).cpu().numpy()[0, 0]
                ...
                pred_disp = self.disp_net(tgt).cpu().numpy()[0, 0]
                predictions[i + 1] = 1 / pred_disp

                if i == len(dataloader) - 1:
                    gt_depths.append(np.load(self.gt_depth_paths[i + 2]))
                    pred_disp = self.disp_net(ref2).cpu().numpy()[0, 0]
